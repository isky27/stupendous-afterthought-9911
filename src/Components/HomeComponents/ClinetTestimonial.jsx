...
function ClinetTestimonial() {
    ...
    const [imageSrc, setImageSrc] = useState(crauserData[0].image);
    const [imageState,setImageState] = useState(0)
    const [crouserPara, setCrouserPara] = useState(crauserData[0].text);
    const [crauserTitle, setCrauserTitle] = useState(crauserData[0].title);
    const [crauserDesignation, setCrauserDesignation] = useState(crauserData[0].designation);

    const rightClick = () =>{
        
      setImageState(imageState+1)
      if (imageState === crauserData.length) {
        setImageState(0);
      }
